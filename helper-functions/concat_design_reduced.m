...
for s=sub_all
    ...
    folder_path_sub_glm_new = fullfile(folder_path_sub, '1st_level_new_reduced');
    
    if ~exist(folder_path_sub_glm_new,'dir')
        mkdir(folder_path_sub_glm_new);
    end

    % concat stim and imag conditions
    condnames(1:6) = [];
    condnames = [{'Stim'}, {'Imag'}, condnames];

    % Initialize new cell arrays to hold the combined columns
    first_three_combined = cell(size(onsets, 1), 1);
    next_three_combined = cell(size(onsets, 1), 1);

    % Loop through each row and concatenate the first three columns and the next three columns
    for i = 1:size(onsets, 1)
        first_three_combined{i} = [onsets{i, 1}, onsets{i, 2}, onsets{i, 3}];
        next_three_combined{i} = [onsets{i, 4}, onsets{i, 5}, onsets{i, 6}];
    end

    % Remove the first six columns
    onsets(:, 1:6) = [];

    % Insert first_three_combined and next_three_combined at the beginning
    onsets = [first_three_combined, next_three_combined, onsets];
